...
from app.errors.services import trigger_django_404, trigger_django_object_does_not_exist, trigger_errors, trigger_exception
...
class AbstractQuery:
    ...
    def set_load_more_params(self):
        if self.params:
            self.limit = int(self.params.get('limit')) - 1 if self.params.get('limit') else None
            self.ORDERING = self.params.get('ordering')
            self.order_by = self.params.get('order_by')
    ...
    def array_to_in_format(self, arr):
    ...
    def exec_query(self):
        ...
        if not self.QUERY: 
            print("Nema queria")
            raise trigger_exception("no_query")
        try:
    ...
    def prepare_exec(self):
        ...
        self.generate_sql()
        if DEBUG:
            self.write_last_sql_query()

    def clear(self):
        pass

    def get_response(self, response_format=True):
        """
        If response formate equal True then response: {"results": [], "message": ""}
        """
        self.exec_query()
        self.clear()
        if response_format:
